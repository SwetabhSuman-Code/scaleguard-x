...
class JWTHandler:
    ...
    def generate_token(
        ...
        token = jwt.encode(claims.to_dict(), self.config.secret_key, algorithm=self.algorithm)

        logger.debug(f"JWT generated for subject={subject}, " f"expires_in={exp_minutes}min")
        ...
        try:
            ...
            if claims.exp:
                ...
                return {
                    ...
                    "scopes": claims.scopes,
                    "issued_at": (
                        datetime.fromtimestamp(claims.iat).isoformat() if claims.iat else None
                    ),
