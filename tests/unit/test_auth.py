...
class TestTokenClaims:
    ...
    def test_claims_to_dict(self):
        ...
        claims = TokenClaims(
            sub="user123", username="john", roles=["operator"], scopes=["metrics:read"]
        )
        d = claims.to_dict()
...
class TestJWTHandlerInitialization:
    ...
    def test_initialization_rejects_invalid_algorithm(self):
        ...
        with pytest.raises(ValueError):
            JWTHandler(JWTConfig(secret_key="test-secret", algorithm="INVALID"))
...
class TestJWTGeneration:
    ...
    def test_token_expiration_time(self):
        ...
        handler = JWTHandler(JWTConfig(secret_key="test-secret", expiration_minutes=120))
        result = handler.generate_token("user123")
...
class TestJWTValidation:
    ...
    def test_validate_token_with_roles(self):
        ...
        handler = JWTHandler(JWTConfig(secret_key="test-secret"))
        result = handler.generate_token("user123", roles=["operator", "viewer"])
        token = result["access_token"]
...
class TestRefreshTokens:
    ...
    def test_generate_refresh_token(self):
        ...
        handler = JWTHandler(JWTConfig(secret_key="test-secret", refresh_expiration_days=7))
...
class TestRBACPermissions:
    ...
    def test_operator_can_scale(self):
        ...
        has_access = rbac.has_access("user123", ["operator"], Permission.SCALING_EXECUTE)
        assert has_access
    ...
    def test_viewer_cannot_scale(self):
        ...
        has_access = rbac.has_access("user123", ["viewer"], Permission.SCALING_EXECUTE)
