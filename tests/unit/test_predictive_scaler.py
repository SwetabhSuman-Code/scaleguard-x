...
class TestPredictiveScalerInitialization:
    ...
    def test_custom_configuration(self):
        ...
        config = PredictiveScalerConfig(pid_setpoint=60.0, spike_probability_threshold=0.5)
    ...
    def test_with_ml_modules(self):
        ...
        scaler = PredictiveScaler(prophet_module=mock_prophet, lstm_module=mock_lstm)
...
class TestPIDComponentScaling:
    ...
    def test_pid_component_high_utilization(self):
        ...
        scaler = PredictiveScaler(PredictiveScalerConfig(pid_setpoint=70.0))
...
class TestProphetIntegration:
    ...
    def test_prophet_missing_module(self):
        ...
        scaler = PredictiveScaler(PredictiveScalerConfig(prophet_enabled=True), prophet_module=None)
        scaler.last_scaling_decision_time = time.time() - 1000
...
class TestLSTMIntegration:
    ...
    def test_lstm_missing_module(self):
        ...
        scaler = PredictiveScaler(PredictiveScalerConfig(lstm_enabled=True), lstm_module=None)
        scaler.last_scaling_decision_time = time.time() - 1000
    ...
    def test_lstm_failure_handling(self):
        ...
        scaler = PredictiveScaler(PredictiveScalerConfig(lstm_enabled=True), lstm_module=mock_lstm)
        scaler.last_scaling_decision_time = time.time() - 1000
...
class TestThrashingPrevention:
    ...
    def test_minimum_scaling_magnitude(self):
        ...
        config = PredictiveScalerConfig(min_scaling_magnitude=0.5, pid_setpoint=70.0)
        scaler = PredictiveScaler(config)
        scaler.last_scaling_decision_time = time.time() - 1000
...
class TestMultiFactorDecision:
    ...
    def test_combined_pid_and_prophet(self):
        ...
        mock_prophet = Mock()
        mock_prophet.predict_next_10_minutes.return_value = {"upper_bound": 85.0}

        scaler = PredictiveScaler(
            PredictiveScalerConfig(pid_setpoint=70.0), prophet_module=mock_prophet
        )
        scaler.last_scaling_decision_time = time.time() - 1000

        decision = scaler.decide_scaling(85.0, recent_data=Mock(), dt=1.0)
    ...
    def test_lstm_boost_on_emergency(self):
        ...
        config = PredictiveScalerConfig(
            pid_kp=1.0, spike_probability_threshold=0.6, spike_scaling_boost=2.0
        )
        scaler = PredictiveScaler(config, lstm_module=mock_lstm)
        scaler.last_scaling_decision_time = time.time() - 1000
