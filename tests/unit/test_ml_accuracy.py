...
class TestProphetVsARIMA:
    ...
    def test_prophet_handles_spikes_arima_cannot(self):
        ...
        assert prediction['spike_probability'] > 0.4, \
            "Prophet failed to detect spike"
        assert 'SPIKE' in prediction.get('warning', '') or \
               'SURGE' in prediction.get('warning', ''), \
    ...
    def test_prophet_captures_trend_change(self):
        ...
        assert prediction['predicted_value'] > 190, \
            "Prophet didn't capture trend change"
    ...
    def test_prophet_returns_ranges_arima_returns_points(self):
        ...
        data = pd.DataFrame({
            'ds': dates,
            'y': np.sin(np.arange(4320) * 2 * np.pi / 288) * 50 + 200
        })
        ...
        print(f"✓ Prophet confidence interval: "
...
class TestMultiHorizonAccuracy:
    ...
    def test_prophet_accuracy_by_horizon(self):
        ...
        for horizon in horizons:
            ...
            print(f"✓ Prophet {horizon}-min horizon: "
...
class TestProphetReadiness:
    ...
    def test_prophet_deterministic(self):
        ...
        data = pd.DataFrame({
            'ds': dates,
            'y': np.sin(np.arange(2016) * 2 * np.pi / 288) * 50 + 200
        })
