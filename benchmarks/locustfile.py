...
class MetricsTaskSet(TaskSet):
    ...
    def post_single_metric(self):
        ...
        with self.client.post("/api/metrics", json=payload, catch_response=True) as response:
    ...
    def post_batch_metrics(self):
        ...
        for i in range(random.randint(3, 8)):
            payload = MetricsPayloadGenerator.generate_payload(node_id=f"worker-batch-{i:02d}")
...
class SpikeMetricsUser(HttpUser):
    ...
    def rapid_fire_metrics(self):
        ...
        for _ in range(random.randint(5, 15)):
            payload = MetricsPayloadGenerator.generate_payload()
            self.client.post("/api/metrics", json=payload, timeout=5)
