...
async def test_throughput_1k_metrics_per_sec(save_benchmark_result, logger):
    ...
    logger.info(f"Results: {results['achieved_rps']} RPS, "
...
async def test_throughput_spike_handling(save_benchmark_result, logger):
    ...
    combined = {
        ...
        "spike_recovery": {
            "latency_increase_ms": spike_results["p99_latency_ms"] - normal_results["p99_latency_ms"],
    ...
    assert combined["spike_phase"]["error_rate"] < 0.10, \
        "Too many errors during spike"
    assert combined["spike_recovery"]["recovered"] or \
           spike_results["p99_latency_ms"] < 1000, \
...
async def test_throughput_sustained_30_minutes(save_benchmark_result, logger):
    ...
    logger.info(f"30-min results: {results['achieved_rps']} avg RPS, "
                f"Error Rate={results['error_rate']:.2%}")
    
    # Should maintain performance over long duration
    assert results["error_rate"] < 0.05, \
        "Too many errors during sustained load"
    assert results["achieved_rps"] > 4500, \
        "Performance degraded over 30 minutes"
