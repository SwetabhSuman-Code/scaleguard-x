...
class LatencyBenchmark:
    ...
    async def measure_api_latency(
        ...
        async with httpx.AsyncClient(base_url=self.base_url, timeout=30) as client:
            ...
            if concurrent:
                ...
                latencies = await self._measure_sequential(client, endpoint, samples)

            self.latencies = latencies

        return self._calculate_percentiles(latencies)

    async def _measure_sequential(
        self, client: httpx.AsyncClient, endpoint: str, samples: int
    ) -> List[float]:
    ...
    def measure_query_latency(self, query_params: Dict, samples: int = 500) -> Dict:
...
async def test_health_endpoint_latency(save_benchmark_result, logger):
    ...
    results = await benchmark.measure_api_latency("/health", samples=1_000, warm_up=10)

    save_benchmark_result("latency_health_endpoint", results)

    logger.info(
        f"Health endpoint - P99: {results['p99_ms']}ms, "
        f"P95: {results['p95_ms']}ms, "
        f"Mean: {results['mean_ms']}ms"
    )

    assert results["p99_ms"] < 50, f"Health endpoint P99 too high: {results['p99_ms']:.0f}ms"
...
async def test_metrics_post_latency_sequential(save_benchmark_result, logger):
    ...
    logger.info(f"POST /api/metrics - P99: {results['p99_ms']}ms, " f"Mean: {results['mean_ms']}ms")

    assert results["p99_ms"] < 500, f"Metrics POST P99 too high: {results['p99_ms']:.0f}ms"
    assert results["success"]
...
async def test_metrics_post_latency_concurrent(save_benchmark_result, logger):
    ...
    logger.info(f"Concurrent POST - P99: {results['p99_ms']}ms, " f"Mean: {results['mean_ms']}ms")

    assert results["p99_ms"] < 700, f"Concurrent POST P99 too high: {results['p99_ms']:.0f}ms"
