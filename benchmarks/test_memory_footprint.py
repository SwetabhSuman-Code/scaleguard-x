...
async def test_memory_under_1k_rps(save_benchmark_result, logger):
    ...
    logger.info(f"1K RPS - Initial: {profile['initial_mb']:.0f}MB, "
                f"Peak: {profile['peak_mb']:.0f}MB, "
                f"Growth: {profile['growth_mb']:.0f}MB")
    
    # Memory shouldn't grow unbounded
    assert profile['growth_mb'] < 300, \
        f"Memory growth too high: {profile['growth_mb']:.0f}MB"
...
async def test_memory_under_5k_rps(save_benchmark_result, logger):
    ...
    profile = results['profile']['memory']
    logger.info(f"5K RPS - Initial: {profile['initial_mb']:.0f}MB, "
                f"Peak: {profile['peak_mb']:.0f}MB, "
                f"Growth: {profile['growth_mb']:.0f}MB")
    
    # Should not exceed 500MB for a single instance
    assert profile['peak_mb'] < 500, \
        f"Peak memory too high: {profile['peak_mb']:.0f}MB"
...
async def test_memory_leak_detection(save_benchmark_result, logger):
    ...
    logger.info(f"1-hour leak test - Growth: {profile['growth_mb']:.0f}MB "
                f"({(profile['growth_mb'] / profile['initial_mb'] * 100):.1f}%)")
    
    # Should grow less than 50% over 1 hour
    growth_percent = (profile['growth_mb'] / profile['initial_mb'] * 100) if profile['initial_mb'] > 0 else 0
    assert growth_percent < 50, \
        f"Possible memory leak: {growth_percent:.1f}% growth in 1 hour"
